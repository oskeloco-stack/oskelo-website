...
_Last updated: 2026-09-11 (Same session, continued again: user reported "some things seem to be missing" after the push — investigated on the live site, the one concrete lead (photography work-card images) turned out to be a lazy-load false alarm, not a real bug; still waiting on specifics. Separately fixed a real bug they reported: `/admin/enhance` rejected most non-JPG/PNG formats outright — now permissive (lets sharp's real decode be the gate, with a friendly message for genuinely unsupported formats like HEIC/RAW), accepts paste-from-clipboard, and the page is wider. Committed as `58237a4`; **not yet pushed** — see Outstanding.)_
...
- [ ] **Pin down "some things seem to be missing"** — reported after the big push, no specifics given yet. The one lead checked (blank photography work-cards) was a lazy-load false alarm, not real. Since the user was actively using `/admin/enhance` live in the same breath, the admin area is at least partly reachable for them — so the `SUPABASE_SERVICE_ROLE_KEY`/`ADMIN_EMAIL` Vercel item right below is NOT confirmed fixed, but also evidently isn't blocking everything. Next session: ask specifically which page and what's absent.
- [ ] **Confirm `SUPABASE_SERVICE_ROLE_KEY` and `ADMIN_EMAIL` are set in Vercel** (Settings → Environment Variables) — flagged as still-needed after the 2026-09-10 push and never explicitly confirmed since. If not set, `/admin/images`, `/admin/content`, `/admin/analytics`, `/admin/messages`, `/admin/enhance` all 500 on the live site (public pages + login are unaffected).
