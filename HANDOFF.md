...
_Last updated: 2026-09-11 (Same session, one more round: Enhance now lets you choose what happens to a corrected photo instead of auto-saving it — Send to Images / Edit manually (brightness, contrast, saturation, sharpen, 90° rotate, with a live CSS-filter preview) / Discard. Also found real evidence the Vercel env var fix from earlier in this session actually worked: two photos (`photo-enhanced.jpg`, `photo-enhanced-2.jpg`) already sit in the live `media` bucket from the user testing it themselves — left untouched, see below. Committed as `c10c401`; **not yet pushed**.)_
